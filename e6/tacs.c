...
void makeAritmeticOpAsm(TAC* tac, FILE* out, char *op) {
   ...
   if(tac->op2->type == SYMBOL_LIT_INT)
        ...
        fprintf(out, "%s _%s(%%rip), %%eax\n", op, tac->op2->text);  
    }
}

void makeBooleanOpAsm(TAC* tac, FILE* out, char *op) {
    if(tac->op1->type == SYMBOL_LIT_INT) {
        fprintf(out, "\tmovl	$%s, %%eax\n", tac->op1->text);
    }
    else {
   ...
   if(tac->op2->type == SYMBOL_LIT_INT)
        fprintf(out, "\tcmpl	%s, %%eax", op, tac->op2->text);
    else {
        fprintf(out, "\tcmpl	_%s(%%rip), %%eax", op, tac->op2->text);  
    }
...
void generateAsm(TAC* first, FILE* out) {
    TAC* tac;
    // Init
